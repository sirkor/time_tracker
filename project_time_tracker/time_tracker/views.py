...
def statistic_view(request, username=''):
    ...
    statistic = Activities.objects.filter(activities_user=request.user.username)
    sum_of_duration = datetime.timedelta(0)
    for stat in statistic:
        if stat.activities_start.month == now.month:
            sum_of_duration += stat.activities_duration

    statistic = Activities.objects.filter(activities_user=request.user.username, activities_type='Работа')
    work_duration = datetime.timedelta(0)
    for stat in statistic:
        if stat.activities_start.month == now.month:
            work_duration += stat.activities_duration

    statistic = Activities.objects.filter(activities_user=request.user.username, activities_type='Остальное')
    ...
    args = {'sum_duration': sum_of_duration, 'work_duration': work_duration, 'other_duration': other_duration,
